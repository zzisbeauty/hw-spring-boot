...
import com.hanwei.asr.mapper.AsrServiceInfoMapper;
...
@Service
@Slf4j
public class AsrServiceInfoServiceImpl extends ServiceImpl<AsrServiceInfoMapper, AsrServiceInfo> implements IAsrServiceInfoService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, AsrServiceInfo asrServiceInfo) {
        QueryWrapper<AsrServiceInfo> queryWrapper = QueryGenerator.initQueryWrapper(asrServiceInfo, paramMap);
        List<AsrServiceInfo> list = list(queryWrapper);
        EasyExcel.write(outputStream, AsrServiceInfo.class).sheet("ASR服务信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, AsrServiceInfo asrServiceInfo) {
        QueryWrapper<AsrServiceInfo> queryWrapper = QueryGenerator.initQueryWrapper(asrServiceInfo, paramMap);
        List<AsrServiceInfo> list = list(queryWrapper);
        //字典值转换
        List<JSON> listJson = commonApi.translateResultByDict(list);
        List<AsrServiceInfo> result = listJson.stream().map(e -> JSON.toJavaObject(e,AsrServiceInfo.class)).collect(Collectors.toList());
        String excelContent = null;
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        EasyExcel.write(outputStream, AsrServiceInfo.class).sheet("ASR服务信息").doWrite(result);
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), AsrServiceInfo.class, new ReadListener<AsrServiceInfo>() {

                private List<AsrServiceInfo> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(AsrServiceInfo data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/AsrServiceInfo.xlsx");
            // 方式一：路径
     ...
     */
    @Override
    public Result<?> removeAsrServiceInfoById(String id) {
        if (StrUtil.isEmpty(id)) {
            return Result.error("删除失败 未找到服务");
        }
        List<AsrServiceConfig> asrServiceConfigList = asrServiceConfigService.list(new QueryWrapper<AsrServiceConfig>().lambda().eq(AsrServiceConfig::getAsrId, id));
        if (null!= asrServiceConfigList && asrServiceConfigList.size() > 0) {
            return Result.error("删除失败 服务已被使用");
        }
        removeById(id);
    ...
    public void untilStatusHandle() {
        //获取所有服务信息
        try {
            List<AsrServiceInfo> serviceInfos = list();
            for (AsrServiceInfo serviceInfo : serviceInfos) {
     ...
     */
    private void updateStatus(AsrServiceInfo serviceInfo) {
        //判断服务信息是否过期
     ...
     */
    @Override
    public boolean save(AsrServiceInfo entity) {
        updateStatus(entity);
     ...
     */
    @Override
    public boolean updateById(AsrServiceInfo entity) {
        if(null == entity.getExpirationTime()){
            AsrServiceInfo oldServiceInfo = getById(entity.getId());
