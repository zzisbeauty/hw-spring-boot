package com.hanwei.asr.service.impl;
...
import com.hanwei.asr.mapper.AsrServiceConfigMapper;
 ...
 */
@Service
public class AsrServiceConfigServiceImpl extends ServiceImpl<AsrServiceConfigMapper, AsrServiceConfig> implements IAsrServiceConfigService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, AsrServiceConfig asrServiceConfig) {
        QueryWrapper<AsrServiceConfig> queryWrapper = QueryGenerator.initQueryWrapper(asrServiceConfig, paramMap);
        List<AsrServiceConfig> list = list(queryWrapper);
        EasyExcel.write(outputStream, AsrServiceConfig.class).sheet("ASR实例配置信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, AsrServiceConfig asrServiceConfig) {
        QueryWrapper<AsrServiceConfig> queryWrapper = QueryGenerator.initQueryWrapper(asrServiceConfig, paramMap);
        List<AsrServiceConfig> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<AsrServiceConfig> result = listJson.stream().map(e -> JSON.toJavaObject(e,AsrServiceConfig.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), AsrServiceConfig.class, new ReadListener<AsrServiceConfig>() {

                private List<AsrServiceConfig> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(AsrServiceConfig data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/AsrServiceConfig.xlsx");
            // 方式一：路径
    ...
    public Result<?> removeAsrConfigInfoById(String id) {
        ...
        AsrServiceConfig asrServiceConfig = getById(id);
        if(Optional.ofNullable(asrServiceConfig).isEmpty()){
            return Result.ok("删除成功");
        }

        // 删除ASR热词信息
        if(StrUtil.isNotEmpty(asrServiceConfig.getHotWordId())){
            Boolean success = asrHotWordService.removeById(asrServiceConfig.getHotWordId());
            if(!success){
                return Result.error(200,"删除失败,热词表删除失败");
            }
        }

        // 删除ASR关键词信息
        if(StrUtil.isNotEmpty(asrServiceConfig.getKeyWordId())){
            Boolean success = asrKeyWordService.removeById(asrServiceConfig.getKeyWordId());
            if(!success){
                return Result.error(200,"删除失败,关键词表删除失败");
            }
        }
    ...
    @Override
    @Transactional
    public Result<?> AddHotWord(String id, AsrHotWord asrHotWord) {
        asrHotWordService.save(asrHotWord);
        AsrServiceConfig asrServiceConfig = getById(id);
        asrServiceConfig.setHotWordId(asrHotWord.getId());
        updateById(asrServiceConfig);
        return Result.ok("配置热词成功");
    }
    ...
    @Override
    @Transactional
    public Result<?> AddKeyWord(String id, AsrKeyWord asrKeyWord) {
        asrKeyWordService.save(asrKeyWord);
        AsrServiceConfig asrServiceConfig = getById(id);
        asrServiceConfig.setKeyWordId(asrKeyWord.getId());
        updateById(asrServiceConfig);
        return Result.ok("配置关键词成功");
    }
    ...
    public Boolean switchStatus(String id,Integer status) {
        AsrServiceConfig asrServiceConfig = getById(id);
        if(null == asrServiceConfig){
            return false;
        }
        asrServiceConfig.setStatus(status);
        updateById(asrServiceConfig);
        return true;
    }
}
