...
import com.hanwei.system.mapper.CategoryInfoMapper;
...
public class CategoryInfoServiceImpl extends ServiceImpl<CategoryInfoMapper, CategoryInfo> implements ICategoryInfoService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, CategoryInfo categoryInfo) {
        QueryWrapper<CategoryInfo> queryWrapper = QueryGenerator.initQueryWrapper(categoryInfo, paramMap);
        List<CategoryInfo> list = list(queryWrapper);
        EasyExcel.write(outputStream, CategoryInfo.class).sheet("分类信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, CategoryInfo categoryInfo) {
        QueryWrapper<CategoryInfo> queryWrapper = QueryGenerator.initQueryWrapper(categoryInfo, paramMap);
        List<CategoryInfo> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<CategoryInfo> result = listJson.stream().map(e -> JSON.toJavaObject(e,CategoryInfo.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), CategoryInfo.class, new ReadListener<CategoryInfo>() {

                private List<CategoryInfo> cachedDataList = new ArrayList<>();
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/CategoryInfo.xlsx");
            // 方式一：路径
