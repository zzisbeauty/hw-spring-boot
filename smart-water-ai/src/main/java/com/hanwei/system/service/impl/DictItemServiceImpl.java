...
import com.hanwei.system.mapper.DictItemMapper;
...
public class DictItemServiceImpl extends ServiceImpl<DictItemMapper, DictItem> implements IDictItemService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, DictItem dictItem) {
        QueryWrapper<DictItem> queryWrapper = QueryGenerator.initQueryWrapper(dictItem, paramMap);
        List<DictItem> list = list(queryWrapper);
        EasyExcel.write(outputStream, DictItem.class).sheet("字典明细表表").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, DictItem dictItem) {
        QueryWrapper<DictItem> queryWrapper = QueryGenerator.initQueryWrapper(dictItem, paramMap);
        List<DictItem> list = list(queryWrapper);
        //字典值转换
        List<JSON> listJson = commonApi.translateResultByDict(list);
        List<DictItem> result = listJson.stream().map(e -> JSON.toJavaObject(e,DictItem.class)).collect(Collectors.toList());
        String excelContent = null;
        ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
        EasyExcel.write(outputStream, DictItem.class).sheet("字典明细表").doWrite(result);
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), DictItem.class, new ReadListener<DictItem>() {

                private List<DictItem> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(DictItem data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/DictItem.xlsx");
            // 方式一：路径
