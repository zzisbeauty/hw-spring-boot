package com.hanwei.process.business.secondsupply.controller;
...
@Slf4j
@RestController
@RequestMapping("/modelinvoking/secondsupply")
public class SecondSupplyController extends BaseController<ModelInvokingInfo, IModelInvokingService> {
	 ...
	 * @return
	 */
	@AutoLog(value = "模型调用-二供数据总览")
	@GetMapping(value = "/dataStatistics")
	public Result<?> dataStatistics() {
		StringBuffer logBuffer = new StringBuffer();
		logBuffer.append("模型调用-二供数据总览");
		try {
			log.info(logBuffer.append("成功").toString());
			ResultForFrontVo resultForFrontVo = secondSupplyService.dataStatistics();
	 ...
	 * @return
	 */
	@AutoLog(value = "模型调用-二供监测总览")
	@GetMapping(value = "/gisStatistics")
	public Result<?> gisStatistics() {
		StringBuffer logBuffer = new StringBuffer();
		logBuffer.append("模型调用-二供监测总览");
		try {
			log.info(logBuffer.append("成功").toString());
			ResultForFrontVo resultForFrontVo = secondSupplyService.gisStatistics();
