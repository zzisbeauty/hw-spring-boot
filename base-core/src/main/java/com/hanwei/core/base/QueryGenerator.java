...
import com.hanwei.core.common.SymbolConstant;
import com.hanwei.core.common.database.DataBaseConstant;
import com.hanwei.core.common.database.MatchTypeEnum;
import com.hanwei.core.common.database.QueryCondition;
import com.hanwei.core.common.database.QueryRuleEnum;
import com.hanwei.core.exception.BaseException;
import com.hanwei.core.oldquery.PostData;
import com.hanwei.core.util.CommonUtils;
...
public class QueryGenerator {
    ...
    private static final String OR = "_or";
    /**
     * 数字类型字段，拼接此后缀 接受多值参数
     */
     ...
     * <br>错误示例:如QueryWrapper<T> queryWrapper = new QueryWrapper<T>(T);
     * <br>正确示例:QueryWrapper<T> queryWrapper = new QueryWrapper<T>();
     * <br>3.也可以不使用这个方法直接调用 {@link #initQueryWrapper}直接获取实例
     */
    private static void installMplus(QueryWrapper<?> queryWrapper, Object searchObj, Map<String, String[]> parameterMapOri) {
        Map<String, String[]> parameterMap = new HashMap<>(parameterMapOri);

        //默认排序处理
        if(!parameterMap.keySet().contains("column")){
            parameterMap.put("column",new String[]{"id"});
            parameterMap.put("order",new String[]{"asc"});
        }
        ...
        List<Object> orValueArr = new ArrayList<>();
        List<String> orColumnArr = new ArrayList<>();
        ...
        for (int i = 0; i < origDescriptors.length; i++) {
            ...
            try {
                ...
                //增加对OR条件的简单支持
                if(parameterMap != null && parameterMap.containsKey(name + OR) && parameterMap.get(name + OR).length > 0){
                    orValueArr.add(parameterMap.get(name + OR)[0]);
                    orColumnArr.add(column);
                    continue;
                }
        ...
        //处理or条件
        if(orColumnArr.size()!=0){
            for(int i = 0;i<orColumnArr.size();i++){
                //根据参数值带什么关键字符串判断走什么类型的查询
                Object value = orValueArr.get(i);
                column = orColumnArr.get(i);
                QueryRuleEnum rule = convert2Rule(value);
                value = replaceValue(rule, value);
                queryWrapper.or();
                addEasyQuery(queryWrapper, column, rule, value);
    ...
    private static void doMultiFieldsOrder(QueryWrapper<?> queryWrapper, Map<String, String[]> parameterMap, Map<String, String> fieldColumnMap) {
        ...
        if (oConvertUtils.isNotEmpty(column) && oConvertUtils.isNotEmpty(order)) {
            ...
            if(columnArray.length != orderArray.length){
                throw new BaseException("请注意，排序字段与排序规则个数不匹配!");
            }
            ...
            for(int index = 0;index<columnArray.length;index++){
                if (orderArray[index].toUpperCase().indexOf(ORDER_TYPE_ASC) >= 0) {
                    queryWrapper.orderByAsc(columnArray[index]);
                } else {
                    queryWrapper.orderByDesc(columnArray[index]);
                }
            }
        }
    }
    ...
    private static String getDbType() {
        return CommonUtils.getDatabaseTypeEnum().getDb();
    }
